...
def train_exe(model_name, phase, cv_idx_tuple, params_obj, opt_model_params=None,
              opt_fit_params=None, tr_df=None, test_df=None,
              trial=None,  acc_local_loss=[]):

  trgt_model = params_obj.model
  trgt_y = params_obj.trgt_y

  if model_name in ["neuralnetclsf", "gnb", "rfclsf", "svc"]:
  ...
  if model_name in ["xgbclsf", "lgbclsf", "catclsf"]:
  ...
  clsf = trgt_model(**model_params)
  if len(fit_params) > 0:
    clsf.fit(X=X_tr, y=y_tr, **fit_params)
  else:
    clsf.fit(X=X_tr, y=y_tr)


  val_pred_proba = clsf.predict_proba(X_val)
  val_pred = clsf.predict(X_val)
  if model_name == "neuralnetclsf":
    val_pred_proba = np.squeeze(val_pred_proba, axis=2)
  ...
  if phase == "opt_phase":
    ...
    if model_name not in ["lgbclsf", "xgbclsf", "catclsf"]:
        test_df = test_df.values.astype(np.float32)
    test_pred_proba = clsf.predict_proba(test_df)
    return loss, model_params, fit_params, val_pred_proba, test_pred_proba
