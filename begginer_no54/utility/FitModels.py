...
from ParamsConfigs import ParamsConfigs
...
class FitModels:
  def __init__(self, tr_df, test_df, trgt_y,
                ...
                make_new_exam=True, use_latest_exam=False):
    ...
    # self.opt_model_params_key = self.cnf_yml.
    # self.opt_fit_params_key = self.cnf_yml.
    ...
    if not self.use_single_model:
        pred_cols = [f"{m}_pred_proba" for m in self.use_lv1_models] #各ベースモデルの予測値をメタモデルの入力とする
        trgt_col = [f"{self.trgt_y}"]
        meta_tr_df = pd.DataFrame(columns=["id"] + pred_cols + trgt_col)
        meta_tr_df["id"] = self.tr_df["id"]
        meta_tr_df[trgt_col] = self.tr_df[trgt_col]
  ...
  def hypara_search(self, model_name, tr_df, n_trials, level_status,
    ...
    # print(self.global_study.user_attrs)
  ...
  def fit_level_one_models(self):
    ...
    for model_name in  self.use_lv1_models:
      self.meta_tr_df[f"{model_name}_pred_proba"] = pd.NA
      self.meta_test_df[f"{model_name}_pred_proba"] = pd.NA
      print(f"///// {model_name}の学習をSTART ! /////")
      tr_df, test_df = self.tr_df.copy(), self.test_df.copy()
      self.level_status = "level_1"
      params_obj = ParamsConfigs(model_name, self.cnf_yml).set_config
      ...
      best_model_params, best_fit_params = self.hypara_search(
          model_name,
          tr_df=tr_df, n_trials=l1_n_trials, level_status=level_status,
          random_state=self.random_state, params_obj=params_obj)

      phase = "best_phase"
      params_obj.phase = phase
      # パラメータの名前の微調整
      if model_name == "neuralnetclsf":
    ...
    #   print(f"///// Best params: {best_params} /////\n")
      ...
      tmp_test_pred_proba_lst = []
      
    #   _params = {}
    #   for k, v in params_obj.model_base_params.items():
    #     _params[k] = v
    ...
    #     _params[k] = v
      for i , cv_idx_tuple in enumerate(cv.split(tr_df)):
        ...
        _, model_params, fit_params, val_pred_proba, test_pred_proba = train_exe(
        ...
        val_pred_proba = pd.DataFrame(val_pred_proba)[1].values
        test_pred_proba = pd.DataFrame(test_pred_proba)[1].values
        self.meta_tr_df.loc[_val_idx, f"{model_name}_pred_proba"] = val_pred_proba
        self.meta_tr_df[f"{model_name}_pred_proba"] = pd.to_numeric(self.meta_tr_df[f"{model_name}_pred_proba"], errors="coerce")
        tmp_test_pred_proba_lst.append(test_pred_proba)
      test_pred_probas = np.column_stack(tmp_test_pred_proba_lst)
      cv_mean_test_pred_probas = test_pred_probas.mean(axis=1)
      print(cv_mean_test_pred_probas)
      self.meta_test_df[f"{model_name}_pred_proba"] = cv_mean_test_pred_probas
      print("\tDone!\n")
  ...
  def fit_level_two_model(self):
    ...
    best_model_params, best_fit_params = self.hypara_search(
          model_name,
          tr_df=tr_df, n_trials=l2_n_trials, level_status=self.level_status,
          random_state=self.random_state, params_obj=params_obj)
    ...
    for i , cv_idx_tuple in enumerate(cv.split(tr_df)):
      ...
      val_pred_proba = pd.DataFrame(val_pred_proba)[1].values
      test_pred_proba = pd.DataFrame(test_pred_proba)[1].values
      self.meta_tr_df.loc[_val_idx, f"{self.trgt_y}_pred_proba"] = val_pred_proba
      tmp_test_pred_proba_lst.append(test_pred_proba)

    test_pred_probas = np.column_stack(tmp_test_pred_proba_lst)
    cv_mean_test_pred_probas = test_pred_probas.mean(axis=1)
    self.meta_test_df[f"{self.trgt_y}_pred_proba"] = cv_mean_test_pred_probas
  ...
  def fit_single_model(self):
    ...
    for i , cv_idx_tuple in enumerate(cv.split(tr_df)):
      ...
      val_pred_proba = pd.DataFrame(val_pred_proba)[1].values
      test_pred_proba = pd.DataFrame(test_pred_proba)[1].values
      self.tr_df.loc[_val_idx, f"{self.trgt_y}_pred_proba"] = val_pred_proba
      tmp_test_pred_proba_lst.append(test_pred_proba)

    test_pred_probas = np.column_stack(tmp_test_pred_proba_lst)
    cv_mean_test_pred_probas = test_pred_probas.mean(axis=1)
    self.test_df[f"{self.trgt_y}_pred_proba"] = cv_mean_test_pred_probas
