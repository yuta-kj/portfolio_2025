...
class FitModels:
  def __init__(self, tr_df, test_df, trgt_y,
    ...
    self.trgt_y = trgt_y
    self.tmp_drop_cols = [x for x in ["id", self.trgt_y] if x in self.tr_df]
    ...
    if not self.use_single_model:
        ...
        meta_tr_df = pd.DataFrame()
        if "id" in self.tr_df.columns:
        	meta_tr_df["id"] = self.tr_df["id"]
        for col in pred_cols:
            meta_tr_df[col] = pd.Series(dtype='float')
        meta_tr_df[self.trgt_y] = self.tr_df[self.trgt_y]
        ...
        for col in pred_cols:
            meta_test_df[col] = pd.Series(dtype='float')
...
#     return df
  ...
  def fit_level_one_models(self):
    ...
    for model_name in  self.use_lv1_models:
      ...
      params_obj.tmp_drop_cols = self.tmp_drop_cols
    #   params_obj.use_time_series = self.use_time_series
      phase = "opt_phase"
    ...
    #   print(f"///// Best params: {best_params} /////\n")
      ...
      if self.cnf_yml.common.use_time_series:
        cv = TimeSeriesSplit(n_splits=cv_n, max_train_size=self.cnf_yml.common.max_train_size, gap=self.cnf_yml.common.gap)  
      else:
      ...
      for i , cv_idx_tuple in enumerate(cv.split(tr_df)):
        ...
        self.meta_tr_df[f"{model_name}_pred"] = self.meta_tr_df[f"{model_name}_pred"].ffill(axis=0)
        # print(meta_tr_df)
        # import pdb; pdb.set_trace()
        tmp_test_pred_lst.append(test_pred)
      
      self.meta_tr_df = self.meta_tr_df.bfill(axis=0)
  ...
  def fit_single_model(self):
    ...
    self.tr_df = self.tr_df.bfill(axis=0)
    test_preds = np.column_stack(tmp_test_pred_lst)
    cv_mean_test_preds = test_preds.mean(axis=1)
